...
class Model:
    """ A generative model of the 2D environment with a 1DoF arm  controlled by 
    ...
    def update(self, state):
        ...
        df = self.h*self.dynamics([self.mu, self.dmu], self.rho)
        self.f += df
        ...
        self.da = \
                - ((sp - self.sp)/(2+self.da)) * (sp - self.mu) / self.sp_sigma \
                - np.dot(np.dot( self.inv_sv_sigma, ((sv - self.sv)/(2+self.da))) , (sv - g(self.mu)))
